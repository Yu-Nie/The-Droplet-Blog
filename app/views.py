...
def post_page(request, slug):
    ...
    # sidebar
    recent_posts = Post.objects.exclude(id=post.id).order_by('-last_updated')[0:3]  # exclude the current post
    top_authors = User.objects.annotate(number=Count('post')).order_by(
        '-number')  # create a field call number that show the total post the author writes
    ...
    context = {'post': post, 'form': form, 'comments': comments, 'is_bookmarked': is_bookmarked, 'is_liked': is_liked,
               'number_of_likes': number_of_likes, 'recent_posts': recent_posts, 'top_authors': top_authors,
